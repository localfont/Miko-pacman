 ...
 *  callback.c
 *
 *  Copyright (c) 2006-2025 Miko Development Team <miko-dev@lists.archlinux.org>
...
static struct miko_progress_bar *totalbar;
...
struct miko_multibar_ui {
	 ...
	 */
	alpm_list_t *active_downloads; /* List of type 'struct miko_progress_bar' */
...
struct miko_multibar_ui multibar_ui = {0};

static int dload_progressbar_enabled(void);
static void init_total_progressbar(void);
static void update_bar_finalstats(struct miko_progress_bar *bar);
static void draw_miko_progress_bar(struct miko_progress_bar *bar);
	...
	switch(event->type) {
		...
		case ALPM_EVENT_PKG_RETRIEVE_FAILED:
			...
			if(total_enabled) {
				...
				draw_miko_progress_bar(totalbar);
...
static bool find_bar_for_filename(const char *filename, int *index, struct miko_progress_bar **bar)
{
	int i = 0;
	alpm_list_t *listitem = multibar_ui.active_downloads;
	for(; listitem; listitem = listitem->next, i++) {
		struct miko_progress_bar *b = listitem->data;
...
static void init_total_progressbar(void)
{
	totalbar = calloc(1, sizeof(struct miko_progress_bar));
	assert(totalbar);
...
static void dload_init_event(const char *filename, alpm_download_event_init_t *data)
{
	...
	struct miko_progress_bar *bar = calloc(1, sizeof(struct miko_progress_bar));
	assert(bar);
 ...
 */
static bool update_bar_stats(struct miko_progress_bar *bar)
{
	int64_t timediff;
...
static void dload_complete_event(const char *filename, alpm_download_event_completed_t *data)
{
	...
	if(data->result == 1) {
		...
		if(multibar_ui.move_completed_up && index != 0) {
			 ...
			 */
			struct miko_progress_bar *former_topbar = multibar_ui.active_downloads->data;
			...
			console_cursor_goto_bar(index);
			draw_miko_progress_bar(former_topbar);
	...
	while(multibar_ui.active_downloads) {
		...
		struct miko_progress_bar *j = head->data;
		if(j->completed) {
			multibar_ui.cursor_lineno--;
			multibar_ui.active_downloads_num--;
