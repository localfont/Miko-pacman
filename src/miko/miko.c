...
/** Output miko version and copyright.
 */
static void version(void)
{
	printf("\n");
	printf(" .--.                  Miko v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	printf("/ _.-' .-.  .-.  .-.   Copyright (C) 2006-2025 Miko Development Team\n");
...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "miko/%s (%s %s) libalpm/%s",
...
static void cl_to_log(int argc, char *argv[])
{
	...
	if(cl_text) {
		alpm_logaction(config->handle, MIKO_CALLER_PREFIX,
				"Running '%s'\n", cl_text);
		free(cl_text);
	}
}
...
int main(int argc, char *argv[])
{
	...
	pm_printf(ALPM_LOG_DEBUG, "miko v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	...
	switch(config->op) {
		case PM_OP_DATABASE:
			ret = miko_database(pm_targets);
			break;
		case PM_OP_REMOVE:
			ret = miko_remove(pm_targets);
			break;
		case PM_OP_UPGRADE:
			ret = miko_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = miko_query(pm_targets);
			break;
		case PM_OP_SYNC:
			ret = miko_sync(pm_targets);
			break;
		case PM_OP_DEPTEST:
			ret = miko_deptest(pm_targets);
			break;
		case PM_OP_FILES:
			ret = miko_files(pm_targets);
			break;
		default:
